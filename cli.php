...
class VendingMachine
{
    ...
    public function createOrder($first_order = true) {
        ...
        if(self::isProductsInMachine() != 0){
            ...
            print "Please get your " . self::getProductTitle($this->selected_product) . ".\n";
            do { 
                print "Your change: " . $this->currency . number_format($this->total_money_input,2) . ". Would you like to choose something else? (Press 'Y' or 'N'):\n";
                flush();
                $input = fread(STDIN,1);
    ...
    private function inputMoney(){
        ...
        do{
            ...
            print "\nProduct cost:       " . $this->currency . number_format($total_money_need, 2);
            print "\nMoney deposited:    " . $this->currency . number_format($total_money_input, 2);
            print "\nRemains to deposit: " . $this->currency . number_format($total_money_need - $total_money_input, 2);
            ...
            if(!is_numeric($input) || !in_array(number_format($input, 2), $this->available_coins, true)){
